...
;
if n_elements(filespec) EQ 0 THEN filespec='o*raw.fits'
...
endelse
print,"Running stisdir.pro with search path ",filespec
...
; lab FSW data, assuming amp D corresponds to ccdgain4
		...
		if float(postarg) eq 0 then begin		; 02nov14
			...
			if m1 ne 0 and m2 ne 0 then begin
				...
				if m1 lt -99 or m2 lt -99 then postarg=string([m1,m2],'(i4,",",i4,"px")')
			endif
