...
;	px by another method in _flt,_crj files!
			...
			if nobs eq 1 and 	$   ; hdcr is OUTPUT of stis_cr
				strpos(fils(ifil),'ocy5') ge 0 then begin
...
; special cases of big CR hit on spectrum, requiring "hand patch"
		if strpos(fils(ifil),'o3tt21040') ge 0 then crej(889,494:496)  $
				=avg([crej(888,494:496),crej(890,494:496)])
		...
		if strpos(fils(ifil),'o4a520040') ge 0 then begin
			crej(329,510)=7600  &  crej(330,511)=7000
			crej(968,509)=9500  &  crej(968,510)=11300  &  endif
		if strpos(fils(ifil),'o61002030') ge 0 then begin
			crej(352:353,510)=11200  &  crej(350:353,511)=9700
			crej(603,511)=2800  &  endif
; 00nov21 - P330E, G750L:
		if strpos(fils(ifil),'o49x28010') ge 0 then begin
			crej(658,510)=6700  &  crej(658,511)=3600
			crej(703,510)=6200  &  crej(703,511)=3600  &  endif
		if strpos(fils(ifil),'o4d101020') ge 0 then begin
			crej(287:289,509)=4400  &  crej(628,509)=5250  &  endif
		if strpos(fils(ifil),'o53002030') ge 0 then begin  ;No CR split!
			crej(822,509)=1650  &  crej(881,511)=238-150  &  endif
		if strpos(fils(ifil),'o8h105030') ge 0 then crej(362,895)=0
		if strpos(fils(ifil),'o8h105030') ge 0 then		$
							crej(362:364,894)=860
		...
		if strpos(fils(ifil),'obc406040') ge 0 then crej(603,516:518) $
									  =110
		if strpos(fils(ifil),'obc406040') ge 0 then crej(348,502:507) $
								          =380
		...
		if strpos(fils(ifil),'obto03050') ge 0 then crej(939,512:513)= $
									3550
		...
		if strpos(fils(ifil),'oceil4060') ge 0 then begin
			crej(206:207,510)=23840  &  crej(207,511)=21550
			crej(429,510)=13000  &  crej(428:429,511)=10840
			crej(433,510:511)=11200  &  endif
		if strpos(fils(ifil),'obc407040') ge 0 then crej(348,504:505)= $
								400
		if strpos(fils(ifil),'obc408040') ge 0 then crej(348,504:505)= $
								170
		if strpos(fils(ifil),'obc410040') ge 0 then crej(348,504:505)= $
								400
		if strpos(fils(ifil),'obc411040') ge 0 then crej(348,504:505)= $
								600
		if strpos(fils(ifil),'obnl09030') ge 0 then crej(348,505)=2750
		if strpos(fils(ifil),'o4pz01030') ge 0 then crej(394,504)=15
		if strpos(fils(ifil),'o4pz01030') ge 0 then		$
						crej(394:395,505:506)=25
		if strpos(fils(ifil),'obto07050') ge 0 then crej(512,505)=1100
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(260:261,506)= $
								65
		if strpos(fils(ifil),'o8vj13010') ge 0 then crej(693,508:509)= $
								50
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(157:159,510)= $
								2200
; ff was 599, but ~5 rows that are ~100 too high:
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(261,512)=0
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(636:637,508)= $
								180
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(636:637,509)= $
								300
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(660:661,514)=0
		if strpos(fils(ifil),'o8vj12020') ge 0 then crej(745:746,512)= $
								150
		...
		if strpos(fils(ifil),'odck02050') ge 0 then begin ; GD153 G430L
			crej(538,510)=450.  &  crej(538:539,511)=150.  &  endif
...
; 2017jul17-intermit. HOT px shows mostly in long GRW exp:
		...
		if strpos(fils(ifil),'odqg01030') ge 0 then 		$
					crej(49,506:520)=crej(49,506:520)-20
		...
		if strpos(fils(ifil),'obc408030') ge 0 then crej(881:882,898)=90
	       ...
	       if strpos(fils(ifil),'odta60010') ge 0 then 		$
						crej(305:306,888:889)=16
	       ...
	       if strpos(fils(ifil),'oe7q01010') ge 0 then		$
					crej(145:146,894)=1710

	       if strpos(fils(ifil),'oe3f01010') ge 0 then		$
				       crej(307:308,767)=20	; Gaia 405_1056
...
;stop	       
		...
		sxaddpar,hdcr,'nextend',nextend,'Orig value. '+		$
				'All IDL spec_* files have nextend=1.'	
		
		sxaddpar,hdcr,'pstrtime',pstrtim,		$
				'predicted obs. start time from SPT file'
		...
		if targ eq 'HD209458' and wfilck eq '' then 		$
			sxaddhist,'No Wavecal for HD209458',hdcr
...
; do not subtract any background... (bkgr still computed and written)
			...
			sxaddhist,'Sub-array inserted into 1024x1024'+  $
					' by preproc.pro',hdcr
			sxaddhist,'No background subtraction.',hdcr
			endif
		e1='0'
; E1 normally at row 511+383=894.
		if strpos(aperture,'E1') ge 0 then $
			e1='383'		; rows of offset, ie 511+383=894
...
; G750L GAIA405_6912: oe3f01020 @ low, oe3f01030 @ center:
		...
		if e1 ne '0' then print,targ,' ',root,' ',mode,' ',	$
						aperture,' @ E1 offset=',e1

		sxaddhist,'Wavecal file: '+wfil,hdcr
		if toffix ne 0 then sxaddhist,'STISWLFIX OFFSET(px)='+	$
					string(toffix,'(f5.2)'),hdcr
...
; G750L fringe FLATS:
	...
	if strpos(mode,'G750L') ge 0 and cenwave eq '7751' 		$
					and ext ne '.noflat'   then begin
...
; ###change? NO-the small slit does not align w/ spectrum @ E1:
		   and e1 eq '0'		 	$	; 20jun26
			...
			if name ge 'o3tt42040_raw' and name le 'o3tt48040_raw' $
							 then begin
				print,'SPECIAL AVG CONTEMPORANEOUS FLAT USED'
				sxaddhist,'SPECIAL AVG CONTEMPORANEOUS FLAT '+ $
								'USED',hdcr
; Read un-norm tung flat from preproc and mrgall
				name='tung-noflat.g750l'	;2014sep14 updat
				if gwidth ge '11' then 			$
				   rdf,'dat/tung-noflat11.g750l',1,d else $
				   rdf,'dat/tung-noflat.g750l',1,d	; 7px
				wd=d(*,0)  &  nd=d(*,1)
				goto,skipcal
				endif
			orignam=name
; find right short slit flat file:
			...
			if name ge 'o45a01030_raw' then		$
					name=replace_char(name,'30_','40_')
			if strpos(name,'o4a5') ge 0 then 		$ ;newl
					name=replace_char(name,'20_','10_')
; Later data - GENERAL Purpose:
			if strpos(name,'o6') ge 0 and strpos(name,'o69u')$
					lt 0 then 		$;newagk,gd71
					name=replace_char(name,'40_','50_')
			
; DEFAULT name DEFAULT DEFAULT. GENERAL purpose: ########################
; FF is for switch of file order. Note : the GE and files+1 in ff. line:
			if orignam ge 'o6ig01060_raw' and 		$
				targ ne 'HD209458' then			$
				fdecomp,fils(ifil+1),dum,dum,name	;DEFAULT

; special cases G750L fringe flat files:
			if orignam eq 'o61001040_raw' then 		$
					name='o61001060_raw'
 			if orignam eq 'o6ig01060_raw' then 		$
					name='o6ig01070_raw'
			if orignam eq 'o4a505010_raw' then              $ ; odd!
                                        name=replace_char(name,'10_','20_')
			if strpos(orignam,'o49x') ge 0 then 		$ ;IRSTD
					name=replace_char(name,'10_','20_')
			if strpos(orignam,'o57t') ge 0 then 		$ ;IRSTD
					name=replace_char(name,'20_','30_')
			if orignam eq 'o49x03020_raw' then 		$
					name=replace_char(name,'20_','10_')
			if orignam eq 'o6i903060_raw' then 		$
					name=replace_char(name,'60_','70_')
			if orignam eq 'o6i904060_raw' then 		$
					name=replace_char(name,'60_','70_');50s
			if strpos(orignam,'o49x130') ge 0 then 		$ ;IRSTD
					name='o49x13050_raw'
			if strpos(orignam,'o49x140') ge 0 then 		$ ;IRSTD
					name='o49x14050_raw'
			if orignam eq 'o5ja040i0_raw' then 		$
					name='o5ja040j0_raw'	; fring Flat
; HD209458
			if strpos(orignam,'o6d2') ge 0 then 	$
					name='o6d2a4040_raw'
			if strpos(orignam,'o6d22') ge 0 or		$
				strpos(orignam,'o6d2b') ge 0 then 	$
					name='o6d2b2050_raw'
			if strpos(orignam,'o6n3') ge 0 then 	$
					name='o6n3a2050_raw'
			if strpos(orignam,'o6n3a4') ge 0 or		$
				strpos(orignam,'o6n304') ge 0 then 	$
					name='o6n3a4050_raw'
					
			if strpos(orignam,'o6ig100d0') ge 0 then 	$
					name='o6ig100k0_raw'
			if strpos(orignam,'o5i') ge 0 then 		$ ;IRSTD
					name=replace_char(name,'50_','60_')
			if orignam eq 'o6i904050_raw' then 		$
;;; not used in findfring			name='o6i904080_raw'	; 52x0.1
;					name='o6i904090_raw'	;  0.3X0.09 E1
					name='o6i9040a0_raw'	;  0.1X0.09 E1
			if strpos(orignam,'o8h2010') ge 0 then 	$;02dec10
					name='o8h2010i0_raw'
			if strpos(orignam,'oa9j01090') ge 0 then 	$;09jul
					name='oa9j010b0_raw'
; 18Nov - two G750L obs:
			if strpos(orignam,'odta08030') ge 0 then $;18nov
					name='odta08050_raw'	  ; 16CYGB
			if strpos(orignam,'odta51050') ge 0 then $;18nov
					name='odta51070_raw'	  ; DELUMI
			if strpos(orignam,'odta12') ge 0 then 	 $ ; 18DEC27
					name='odta12070_raw'	  ; eta uma
			if strpos(orignam,'odta19') ge 0 then 	 $ ; 19jan11
					name='odta19070_raw'	  ; 109vir
			if strpos(orignam,'odta07') ge 0 then    $ ; 19jun19
					name='odta07050_raw'	  ; 18sco
			if strpos(orignam,'odta72') ge 0 then    $ ; 19jun19
					name='odta72070_raw'	   ; ETA1DOR
			if strpos(orignam,'odta03') ge 0 then    $ ; 19jun19
					name='odta03070_raw'	   ; HD128998
			if name eq orignam then begin		; idiot check
				print,'IDIOT ck: g750l flat in preproc is data'
				stop  &  endif
			flat=disk+dir+name+'.'+qual
			sxaddhist,'G750L fringe flat & gwidth='+name	$
				+' '+string(gwidth),hdcr
...
; obc407050=fringe flat for obc407040,etc have hot pixel:
			if strpos(fils(ifil),'obc407040') ge 0 then 	$
				pflat(348,505)=400
			if strpos(fils(ifil),'obc408040') ge 0 then 	$
				pflat(348,505)=240
			if strpos(fils(ifil),'obc411040') ge 0 then 	$
				pflat(348,505)=360
			if strpos(fils(ifil),'obnl09030') ge 0 then 	$
				pflat(348,505)=675
			if strpos(fils(ifil),'obc406040') ge 0 then 	$
				pflat(348,505)=240
			if strpos(fils(ifil),'obc406040') ge 0 then 	$
				pflat(603,511)=6000
			if strpos(fils(ifil),'obc406040') ge 0 then 	$
...
; pflfil='DEF' means tch16087o_pfl.fits for post-sm4? nope x6417094o_pfl is used
			calstis,9999,data=pflat,header=hp,'apttab=NONE,'+     $
		   	'pfltfile='+pfltfil+',trace=0,soffset='+e1+','+	$
			'targoffs='+string(-toffix*.0507)+','+		$
			'gwidth='+gwidth+',ctecorr=0,'+			$
			'hrepair=-.04,outspec=none',autowave=toffset,	$
			hspc,orders,wd,fd,eps,err,gross,back,nd
...
;reduce tung fringes by 11%. 2014jun19-No! only by ~2% for .8 or 1.2 high fringe
			...
			print,'G750L SHORT SLIT pflat derived from ',name,  $
				' gwidth=',gwidth
...
; 2019apr4 - mod from gwidth=7 to current gwidth:
			print,'No short Slit Fringe Flat. Do Calstis,'+	$
				' findfinge.pro at row offset=',e1

	   	 calstis,9999,data=crej,header=hdcr,errin=crerr,'apttab=NONE,'+$
           		'darkfile=def,gwidth='+gwidth+',bdist=300,'+ 	$
		   	'trace=0,soffset='+e1+','+			$
; Fails.		'senstab='+senstab+','+ 			$;20jul
			'dist1='+dist1+',pfltfile=none,hrepair=-.04,'+	$
			'targoffs='+string(-toffix*.0507)+','+		$
			'outspec=tmp.fits',autowave=toffset,		$
...
;	bdist='88', as bupper is 44px from hz43b, which is 44 px from HZ43!
		if targ eq 'HZ43' and mode eq 'G750L' and 		$
				root ne 'o69u07030' then begin
			dist1='10'  &  	bdist='88'  &  endif ; 3 spectra
...
; 2019mar28-new g230lb & g430l defaults. Prefer gwidth=11 for sat wide gwidths
		if long(gwidth) ge 11 and mode eq 'G230LB' then		$
						senstab='sens11_g230lb.fits
		if long(gwidth) ge 11 and mode eq 'G430L' then		$
						senstab='sens11_g430l.fits
		if long(gwidth) ge 11 and mode eq 'G750L' then senstab=	$
							'sens11_g750l.fits'
...
; pixel based cte corr test:
		if strmid(filtyp,0,2) eq 'fl' or filtyp eq 'cte' then begin
			fltparam=					$
		      ',biasfile=none,darkfile=none,pfltfile=none,lfltfile=none'
			if filtyp eq 'flc' or filtyp eq 'cte' then	$
				sxaddpar,hdcr,'pixelcte','YES',		$
				after='RAD_VEL','From px-based CTE corr flcfile'
			endif

; CCD processing: MAIN CALL---- 
		if strpos(root,'ocy5') eq 0 then begin		;2020dec9
			ext='-'+mode+'-'+targ+'.fits' 		;14141 Gaia hot
			if targ eq 'GJ-894.3' then ext='-'+mode+	$
						'-FEIGE110.fits'
			endif
		if strpos(root,'oe3f') eq 0 and targ ne 'CCDFLAT'then	$
				ext='-'+mode+'-'+star(ifil)+'.fits' ; Gaia 15816
...
;	fresh IDL session. Just rerun to see the /displ.
		...
		calstis,9999,data=crej,header=hdcr,errin=crerr,		$
...
;		 ',soffset=0,subback=0',	$	;for wavecal
	         'subback='+subkg+',dist1='+dist1+','+		        $
	         'gwidth='+gwidth+',bdist='+bdist+',senstab='+senstab+','+ $
	         'pfltfile='+pfltfil+',trace=0,soffset='+e1+','+  	$
	         'targoffs='+string(-toffix*.0507)+',hrepair='+hotpix+  $
	         'outspec='+subdir+'/spec_'+root+ext+fltparam,		$
		 autowave=toffset,					$
		 hspc,orders,wave,flux,eps,err,gross,back,net,pos,image
...
; Special Gaia bkg adjustments:
		if (strpos(fils(ifil),'oe3f06010') gt 0 or 		$
				...
				and aper(ifil) eq '52X2low' then begin
			z=mrdfits(subdir+'/spec_'+root+ext,1,hdr)
			net=z.net
			bkg=z.gross-net
			if strpos(fils(ifil),'oe3f06010') gt 0 then 	$
					net(491)=0.05	; fix bad dark subtr
			if strpos(fils(ifil),'oe3f03010') gt 0 then	$
					net=net-0.02	;bad bkg drop @ spectrum
...
; Gen. purpose gac11:
			...
			print,'GAC corr for ',gacaper(ind),' ',		$
				gacmode(ind),' gwidth=',gwidth
...
; Special case of using orig Proffit as updated for G230LB&G430L 52X2E1 2011mar:
			if gwidth eq '7' and mode eq 'G430L' then 	$
				linterp,gacwl7(*,ind),gacth7(*,ind),	$
						z.wavelength,thcorr	$
			    else					$
; General case of using my gwidth=11 E1, G230LB & G430L (only) update of 2019mar
;	ie G750L E1 gets orig proffitt result.
				linterp,gacwl(*,ind),gacth(*,ind),	$
						z.wavelength,thcorr
			z.net=net/thcorr
			z.gross=z.net+bkg
			if gwidth eq '7' and mode eq 'G430L' then 	$
				sxaddhist,'GAC Corr of NET & FLUX per '+$
					sxpar(hgac70,'filename'),hdr	$
		    	       else					$
				sxaddhist,'GAC Corr  of NET & FLUX per '$
			  		+sxpar(hgac0,'filename'),hdr
			mwrfits,z,subdir+'/spec_'+root+ext,hdr,/create  ;fix net
...
; scat lite is subtracted in stisflx for G230LB
		...
		sxaddhist,'NET is NOT corrected in this file, except '+	$
						'for GAC at E1.',hdr
		mwrfits,z,subdir+'/spec_'+root+ext,hdr,/create ;fix flux
skipfixes:
...
; 02mar26 - G140L on repeller wire at center before ~jul1, 1997;
		if mode eq 'G140L' and float(date) gt 1997.181 then begin		; See dispersion.doc
			if sxpar(hd,'OMSCYL1P') lt 800 then		$
; 02mar18 corr. for -3, bigger wls. 02sep11 - should switch to targoffs??:
				toffset.offsets=toffset.offsets-0.15	$
; 02mar18 corr. for +3, smaller wls:
			   else 					$
				toffset.offsets=toffset.offsets+0.15
			sxaddhist,'G140L wavelengths corr by 0.15px for'+ $
				' MSM offset of 3 arcsec',hd
			endif
...
; 00nov21 - fix for mult MAMA readout, eg. o47z01060,40 P330E:
		...
		if crsplit ne 1 then begin
			print,' *** WARNING *** MAMA CRSPLIT=',CRSPLIT,	$
				' FOR ', FILS(ifil),' ********************'
			infil=0  &  exptime=0  &  integ=0
...
; soffsets for M33 - UIT339: G140L=-137
		if root eq 'o5co02020' or root eq 'o5co02030' then 	$
			soffst='-137,dist1=20'
...
; 2021mar22 - weak GD71 G140L vignetting obs:
		if strpos(root,'o5300') eq 0 then  begin
			mincnt='50'  &  dist1='500'  &  endif
; MAIN MAMA EXTRACTion
		calstis,9999,data=infil,header=hd,errin=errin,udl=udl,	$
	   		'targoffs='+string(-toffix*.0246)+','+		$
			'pfltfile='+pfltfil+',bdist='+bdist+','+	$
			'soffset='+soffst+',gwidth='+gwidth+ ','+	$
...
;02aug5-new defaults:'bwidth=11,b_order='+bkgfit+',b_mean1=1,b_mean2=1,'+$
	         	'outspec='+subdir+'/spec_'+root+ext+','+	$
	   		'trace=0',autowave=toffset,			$
	   hspc,orders,wave,flux,eps,err,gross,back,net,pos,image
		endelse			; data are now extracted for CCD & MAMAs

	print,'Main CALSTIS: FINISH: ',ifil,' of ',n_elements(fils)-1,	$
					form='(a,i4,a,i4,//)'
...
; - need larger search range for E1. automate 02nov12. 06jul31-backwards. Fix:
		if strpos(aperture,'E1') lt 0 then			$
		      hrs_offset,tmp(650:935),flat(650:935),offset,0,5	$;normal
		   else							$
		      hrs_offset,tmp(650:935),flat(650:935),offset,0,16	 ; E1
...
;			wfix=[5000,8100,8300,8500,8700,8900,9100,9300,	$
;					9500,9700,9900,10100,10300]
;			ffix=[1,1.,0.99714,0.99043,0.98973,0.990415,0.996982,$
;				1.00279,1.00643,0.990736,0.973327,0.96263,0.965]
